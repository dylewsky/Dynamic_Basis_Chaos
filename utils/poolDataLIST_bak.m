...
for i=1:nVars
%    yout(ind,1) = yin(i);
    yout{ind,1} = yin(i); %modified from original code
    ind = ind+1;
end
...
if(usesine) %this block has been fixed from the published version (previously neglected to produce separate sin(k*x) and sin(k*y) terms, e.g.)
    for k=1:10
        for j=1:nVars
            yout{ind,1} = ['sin(',num2str(k),'*',yin{j},')'];
            ind = ind + 1;
        end
        for j=1:nVars
            yout{ind,1} = ['cos(',num2str(k),'*',yin{j},')'];
            ind = ind + 1;
        end
    end
end
